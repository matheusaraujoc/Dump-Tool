...
class UnityDumpToolComplete:
    def __init__(self, root):
        self.root = root
        self.root.title("Unity Dump Tool V33 - Complete (Fixed)")
        ...
        # Regex Poderosos (Baseados na V29 Estável)
    ...
    def build_launcher(self):
        ...
        ttk.Label(frame, text="Unity Dump Tool V33", font=("Segoe UI", 24, "bold")).pack(pady=20)
        ttk.Label(frame, text="Versão Completa: Colunas + Projetos + JSON Externo", foreground="blue").pack(pady=10)
    ...
    def build_main_ui(self, is_project_mode):
        ...
        ext_menu.add_command(label="📥 Importar de JSON", command=self.import_json_external)
    ...
    # =========================================================================
    # LÓGICA DE ARQUIVO E PARSING (REGEX-BASED PARA ESTABILIDADE)
    # =========================================================================
    def escape(self, t): return t.replace('\n', '\\n').replace('\r', '\\r')
    def unescape(self, t): return t.replace('\\n', '\n').replace('\\r', '\r')
        ...
        if f:
            ...
            for e in self.entries:
                if e['unique_key'] in data:
                    n = self.escape(data[e['unique_key']])
                    if n != e['current']: e['current']=n; e['marked']=True; c+=1
            self.populate_list()
            if self.mode=="project": self.sync_ram()
            messagebox.showinfo("OK", f"{c} atualizados")
    ...
    def generate_export_content(self, filename, edits_map):
        ...
        if self.file_type_detected == "xml":
            ...
            modified = self.re_row.sub(row_sub, clean)
             ...
             if k in edits_map: modified = edits_map[k].replace('\n', '\\n').replace('\r', '\\r')
            ...
            def rep_topic(m):
                ...
                state['i'] += 1
                if eid in edits_map: return f"{m.group(1)}{edits_map[eid]}{m.group(3)}"
                return m.group(0)
